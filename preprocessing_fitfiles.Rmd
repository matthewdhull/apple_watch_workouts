...
df <- read.csv('fit.csv', header=TRUE, sep=",")
```
...
```{r message=FALSE, warning=FALSE, results='hide', warnings=F, echo=F}
# average running actual calories
ggplot(subset(actual_cals, workoutType=='running'), mapping = aes(estTotalEnergyBurned)) + 
  geom_density(mapping=aes(totalEnergyBurned, fill='red'), data=subset(df,workoutType=='running'),alpha=.7,color=NA) +
  geom_density(aes(fill='blue'),alpha=.7,color=NA) +
  scale_fill_identity(name='', guide = 'legend', labels=c('calc cals','actual cals')) +
  scale_colour_manual(name = 'the colour', values =c('blue'='blue','red'='red')) +  
  theme_bw() +
  xlab("Calories") +
  ggtitle('Total and Calculated Energy Burns - Running')
```
